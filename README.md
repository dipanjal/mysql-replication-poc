...
In this documentation, we will cover
 ...
 4. Sending Request from a Spring Boot Application.
 5. Observe the Query Execution in the Master and Slave's General log

# Problem Discussion
let's say, you have a single relational databases instance and you probably building a web application. 
so you need a web server or an API framework such as spring boot, django, nodejs bla bla 
and your custom application that essentially speaks to HTTP protocol that 
talks to the browser's or some clients to execute api endpoints and those apis eventually requesting to a database for Reading and Writing.
But what's the problem here ?

Assume, your database is getting lots of reads/write requests For example: `SELECT * FROM products WHERE price < 5000`
But after some period of time you have noticed the read query is not much faster as earlier, it's taking some times which may reduce the edxperience. Why ? Because your product tables is getting larger in course of time and when you are executing the query, it's doing a `Full Table Scan` ( Linear search operation Time Complexity `O(n)` ). So you say, let's add an **Index** to `price` column and your beautiful databse will arrange it in a `Balanced Binary Tree` or a `B-Tree` where the Tree *Balances* itself as a new record *Inserted*, *Deleted* or the existing column value is *Updated* each time, to make the *Read* operations faster in `O(log n)` Time Conplexity where **“n” is the total number of elements in the B-Tree**. But the balancing has a cost, besides the renge query (`SELECT job FROM products WHERE price BETWEEN 5000 AND 10000`) is not effecient in `B-Tree`, there is where `B+ Tree` comes into the picture. But still, what if you have 1 millions of records in the `products` table and you just have inserted a new record and bam. Your DB is doing so much work to `re-balance` the large Tree.
So, What can you do now ? Ok, You can do **Partition** you table based on `id`. Partitioning provides a technique to subdivide objects into smaller pieces. It actually breaks the huge table into many different partiotion tables by range depending on the partiotion key, in our case `id` and these partition tables are again indexed by the `KEY, VALUE` pair where the *partition key* is the KEY and *partition table reference* is the VALUE
For example: id between 1 to 20000 falls into partition1  and ids grater than 20000 goes into the another partition, and so on and so forth. But don't worry guys, these partitions are managed by the Database itself. It knows to get result for the for the Specific *Read* query (`SELECT * FROM products WHERE id = 18`) in which partition it needs to look for. So that's can be a solution to reduce the Tree Balancing Cost, because now as you can feel the Search space is way much smaller than before so the `Balanced B+ Tree` is. Great peorblem solved, but no. Now assume you have a thousands of concurrent users who are fetching and reading from you Database and you Database server is dealing with a huge number of concurrent TCP connections. Your single Database instance is junked up with these enourmous number of concurrent requestes and it might be ran out of it's `QPS (Query Per Second)` limit. Here, **Replication** comes into the space. 

## DB Replication

Replication is a kind of Horizontal Scaling making the same copy of you Full Database and Distribute them in **Master/Slave** architecture where the **Master** deals with all the **Write** operations and *Periodically Updates* it's **Slave** Replicas, will Handle only the **Read** queries. So your Database load is Distributed now. But remember, the *Slaves* must be **Consistent** with the *Master* so there must be a Replication Strategy.

### How the Replication works from Master to Slaves?
After any Write Query (Insert. Update, Delete) executed in the Master, the DB somehow Replicate the changes to teh Slaves. The Master needs to trigger a change event and the Slaves will Get the Changes from the Event and Update themselves.

Idea 1: How about Streaming the Statements?
So Basically, We will be Streaming the Query Statements so the Slaves will Execute these Query Statements inside themselves. Well. this can make the Replication **Inconsistent**. 
Let's Assume you can executed a Query 
```sql
...
So, When any Write Query executes in the Master DB, the change information is storred into the Bin Log. After that the Master will transfer these files toward the Slave Databases **asynchronusly** and Slaves will change their states according to the bin logs.
There are other replication staratagies like Synchronus, Asynchronus and Semi-Asynchronus, but Mysql do Asynchronus replication by default so we are gonna use this for now.

But another problem. How will you distribute the traffics to the appropriate Databases ?
Since you have a multiple instances of you same database depending on *Read/Write* purpose how your application can differenfiate when to go to the Read Replica and when to the Master. 
...
So we need a Reverse Proxy to solve this problem. It sits between our Application and Database and Load Balance the Request to the different DB instances based on our operation types (Read/Write). But how the Proxy will distinguish the Request Type ? The answere is the Proxy must be **SQLAware**. We are going to use `ProxySQL` here.

## ProxySql

![Blank Diagram](./demographic.png)

`ProxySQL` is an Opensource SQLAware Reverse Proxy unlike HTTP or TCP proxy (Nginx, HAProxy etc) it can distinguish the Read/Write operations and deliver the packet to the Specific Database Replica either it's Master or Slave. 
...
By the way, ProxySQL can be used in other DBs like Postgres as well.

## MySQL Replica Configuration
Here is a good read to get the insight of [Mysql Replication Configuration](https://www.digitalocean.com/community/tutorials/how-to-set-up-replication-in-mysql)
I used MySQL 5.7 to prepare one master and two slaves, and set up replication settings. Here is the `docker-compose` I'm using
...
Let's up the docker containers and check the Master and Slave status. `docker-compose up -d`
...
...
```
As you can se the `Slave_IO_Running: Yes, Slave_SQL_Running: Yes` means the slave is working properly.

Also `Master_UUID: 9618dc00-6f2a-11ec-a895-0242ac120002` it's connected successfully with the master. If the Slaves fails to connect to the master then run the ./clean-up.sh. it will gracefully shutdown the containers and clean the master, slave data directories and start the contains in `-d` demon mode
...
```
Now Let's put the proxySql into our existing docker-compose file
...
## Showtime
All our tedious configuration is done and lets open 3 terminals 1 for master and other 2 for slaves and place them side by side so that you can monitor them all together. Try to run some Read/Write query from your Spring Boot application or any Database Client like Mysql Workbench and Monitor the General Logs of Master and Slaves
